using System;
using Company.Entities.MostExplicit;
...
{
	[TestFixture]
	public class FormatWriterTests_ForMostExplicit
	{
		[Test]
		...
		[Test]
		public void Write_OutputsTheDesiredResult_ForFieldThatHasTheRightMaxLengthAlready()
		{
			InterchangeControlLoop element = new InterchangeControlLoop();
			element.InterchangeSenderID = 615156491;
			element.InterchangeDate = new DateTime(2008, 1, 25);
			element.InterchangeTime = DateTime.Now.Date.AddHours(7).AddMinutes(38);
			element.InterchangeControlNumber = 123456789;
			element.UsageIndicator = 'T';

			string actual = FormatWriter.Instance.WriteSingle(element);

			Assert.AreEqual("ISA*00*          *00*          *30*615156491      *ZZ*EYEMED         *080125*0738*^*00501*123456789*0*T*:~" + Environment.NewLine, actual);
		}
